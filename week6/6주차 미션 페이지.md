...
public class ReviewRepositoryImpl implements StoreRepositoryCustom {
...
public class ReviewRepositoryImpl implements StoreRepositoryCustom {
	...
	private final JPAQueryFactory jpaQueryFactory
	private final QReview review = QReivew.review
	
	public void dynamicReviewInsertQuery (String body, Float score) {
			jpaQueryFactory.insert(review)
					.columns(review.body, review.score)
					.values(body, score);
			return;
	}
}
...
필요한 Q 엔티티 클래스는 Review 정도이다. JPAQueryFactory의 insert를 사용한다. 
...
public class HomeRepositoryImpl implements HomeRepositoryCustom {
	...
	public List<HomeDTO> dynamicHomeSelectQuery (String regionName, int cursor) {
			...
			.join(store).on(mission.store.id.eq(store.id))
			.join(region).on(mission.region.id.eq(region.id))
...
public class MemberRepositoryImpl implements MemberRepositoryCustom {
	...
	public MemberDto dynamicFindMemberById(Long memberId) {
	   return jpaQueryFactory
	          .select(Projections.constructor(
	              MemberDto.class,
	              member.nickname,
