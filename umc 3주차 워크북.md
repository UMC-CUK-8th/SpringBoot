...
[2주차 기본 미션 답안지 및 테스트용 데모데이터](https://www.notion.so/2-1b7b57f4596b8136b66edb07069d63d2?pvs=21)
...
[Chapter 3. API URL의 설계 & 프로젝트 세팅 - Node.js](https://www.notion.so/Chapter-3-API-URL-Node-js-1b7b57f4596b81e0ad06ec0f0ad72790?pvs=21)
...
[시니어 미션](https://www.notion.so/1b7b57f4596b81dca6b5e4eef5a5c7bd?pvs=21)
...
![umc 3주차api설계 미션1.png](umc_3%EC%A3%BC%EC%B0%A8api%EC%84%A4%EA%B3%84_%EB%AF%B8%EC%85%981.png)

![umc 3주차api설계 미션2.png](umc_3%EC%A3%BC%EC%B0%A8api%EC%84%A4%EA%B3%84_%EB%AF%B8%EC%85%982.png)

1. 홈화면 api설계는 get을 이용하여 데이터를 호출했고 홈화면에 뜨는 현재 지역과 지역 내 진행한 미션수 를 보여주기 위해 location과 mission_count를 호출하였다.
2. 마이페이지 api설계는 회원이름,아이디,누적 포인트의 정보가 필요하므로 get을 통해 호출하였다.
3. 리뷰 작성은  post를 통해 서버에 전달했고 json형태에 정보를 전달하였다.
4. 미션 목록 조회는 get을 통해 데이터를 호출했고 그 과정에서 미션완료 한것은 is_completed = 1의 데이터를 호출하고 미완료값은 is _completed = 0 으로 호출하였다.
5. 미션 성공 누르기는 성공을 누름으로써 is_completed 값이 1로 바뀌는 patch를 사용했다.
6. 회원가입하기는 post를 사용하여 데이터를 전달했다.
...
[3주차](3%E1%84%8C%E1%85%AE%E1%84%8E%E1%85%A1%201b7b57f4596b81a6be3efa374c464fc2.csv)
